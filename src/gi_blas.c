...
{
	double absa = fabs(*a), absb = fabs(*b), scale = absa + absb;
	if(scale == 0.0)
	{
		*c = 1.0;
		*s = *a = *b = 0.0;
	...
	{
		double r, as = *a / scale, bs = *b / scale;
...
void drot(int n, double *x, int incx, double *y, int incy, double c, double s)
{
	...
	else if(incx == 1 && incy == 1)
	{
		...
		for(; i<m; i+=4)
		{
			register double temp = c*x[i] + s*y[i];
			y[i] = c*y[i] - s*x[i];
			x[i] = temp;
	...
	else
	{
		...
		{
			register double temp = c*x[ix] + s*y[iy];
			y[iy] = c*y[iy] - s*x[ix];
			x[ix] = temp;
		}
	}
...
double dnrm2(int n, const double *x, int incx)
{
	...
	{
		double scale = 0.0, ssq = 1.0, absxi, quot;
		int ix = 0;
		if(incx != 1)
			n = (n-1) * incx;
		for(; ix<n; ix+=incx)
	...
	if(trans == 'N')
	{
		if(incy == 1)
		{
			for(j=0,jx=kx; j<n; ++j,jx+=incx)
			{
				if(x[jx] != 0.0)
				{
					register double temp = alpha * x[jx];
		   ...
		   const double *x, int incx, double beta, double *y, int incy)
{
	...
	if(uplo == 'U')
	{
		if(incx == 1 && incy == 1)
		{
			for(j=0; j<n; ++j)
			{
				register double temp1 = alpha * x[j];
		...
		else
		{
			...
			{
				register double temp1 = alpha * x[jx];
				register double temp2 = 0.0;
		  ...
		  const double *y, int incy, double *A, int lda)
{
	...
	if(incx == 1)
	{
		for(j=0,jy = (incy>0) ? 0 : (-(n-1)*incy); j<n; ++j,jy+=incy)
		{
			if(y[jy] != 0.0)
			{
				register double temp = alpha * y[jy];
				for(i=0,ij=lda*j; i<m; ++i,++ij)
					A[ij] += x[i] * temp;
			}
		}
	...
	if(uplo == 'U')
	{
		if(incx == 1)
		{
			for(j=0; j<n; ++j)
			{
				if(x[j] != 0.0)
				{
					register double temp = alpha * x[j];
					for(i=0,ij=lda*j; i<=j; ++i,++ij)
						A[ij] += x[i] * temp;
				}
			}
	...
	if(uplo == 'U')
	{
		if(incx == 1 && incy == 1)
		{
			for(j=0; j<n; ++j)
			{
				if(x[j] != 0.0 || y[j] != 0.0)
				{
					register double temp1 = alpha * y[j];
					register double temp2 = alpha * x[j];
					for(i=0,ij=lda*j; i<=j; ++i,++ij)
		...
		else
		{
			for(j=0,jx=kx,jy=ky; j<n; ++j,jx+=incx,jy+=incy)
			{
				...
				{
					register double temp1 = alpha * y[jy];
					register double temp2 = alpha * x[jx];
					for(i=0,ix=kx,iy=ky,ij=lda*j; i<=j; ++i,ix+=incx,iy+=incy,++ij)
		...
		if(transa == 'N')
		{
			for(j=0,ij0=0; j<n; ++j,ij0+=ldc)
			{
				...
				for(l=0,lj=ldb*j; l<k; ++l,++lj)
				{
					if(B[lj] != 0.0)
					{
						register double temp = alpha * B[lj];
		...
		if(transa == 'N')
		{
			for(j=0,ij0=0; j<n; ++j,ij0+=ldc)
			{
				...
				for(l=0,jl=j; l<k; ++l,jl+=ldb)
				{
					if(B[jl] != 0.0)
					{
						register double temp = alpha * B[jl];
						for(i=0,ij=ij0,il=lda*l; i<m; ++i,++ij,++il)
							C[ij] += temp * A[il];
					}
				}
