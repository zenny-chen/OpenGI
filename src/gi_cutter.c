...
GIint GICutter_initial_gim(GICutter *cutter, GIMesh *mesh)
{
    ...
    /* create patch */
    mesh->patches = pPatch = GI_CALLOC_ARRAY(sizeof(GIPatch), 1);
    pPatch->id = 0;
    mesh->patch_count = 1;
    pPatch->next = pPatch;
    pPatch->mesh = mesh;
    ...
    else
    {
        ...
        if(cutter->straighten && pRoot->cut_degree > 2)
        {
            ...
            while(qVertices.size)
            {
                ...
                pVWork = GIDynamicQueue_dequeue(&qVertices);
...
*/
                        ...
                        while(qFringe.count)
                        {
                            pEdge = GIHeap_dequeue(&qFringe, &dOldDist);
...
//						dMinDist *= 0.5;
                        ...
                        while(pVertex != pVEnd)
                        {
                            ...
                            pVertex = GIHeap_dequeue(&qFringe, NULL);
                        }
...
GIboolean GIPatch_valid_parameterization(GIPatch *patch)
{
    ...
    do
    {
        dDist = GIvec2d_dist_sqr(pParam->params, corner);
    ...
    dOldDist = GIvec2d_dist_sqr(pParam->params, corner+j);
    while(pParam != patch->corners[0])
    {
        dDist = GIvec2d_dist_sqr(pParam->params, corner+j);
        if(dDist > dOldDist)
        {
            if(patch->corners[i])
            {
