    ...
    for(i=0; i<count; i+=3)
    {
        ...
        for(j=0; j<3; ++j, ++bidx)
        {
            ...
            if(!pVertex || !indices)
            {
                ...
                pVertex = GIHash_find(&hVectorVertexMap, fvec);
                if(!pVertex)
                {
                    /* create new vertex */
...
void GIAPIENTRY giGetMeshiv(GIenum pname, GIint *params)
{
    ...
    case GI_ACTIVE_PATCH:
        *params = pMesh->active_patch != NULL ? pPatch->id : GI_ALL_PATCHES;
        break;
                                 ...
                                 GIuint *indices)
{
    ...
    GIfloat *dst, *fsrc;
    GIuint i, j, a, c, uiAttribSize = 0, vidx = 0;
    ...
    *(GIuint*)pKey = uiAttribSize;
    ...
    do
    {
        ...
        for(i = 0, pHalfEdge = pFace->hedges; i < 3; ++i, pHalfEdge = pHalfEdge->next)
        {
            ...
            idx = (uintptr_t)GIHash_find(&hAttribIndex, pKey);
            if(!idx)
            {
                idx = ++*vcount;
