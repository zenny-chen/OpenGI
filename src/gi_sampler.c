...
//		glClear(GL_COLOR_BUFFER_BIT);
        ...
        if(bGLSL)
        {
            ...
            if(sampler->attrib_mode[i] == GI_SAMPLE_TEXTURED)
            {
                ...
                pGL->_glUniform1i((intptr_t)GIHash_find(&pGL->gim_sampler[
...
void GITexture_construct(GITexture *texture, GIuint gl_texture, GIuint dim)
{
    ...
    texture->data = (GIfloat*)GI_MALLOC_ALIGNED(GI_SSE_SIZE(iSize * 4 * sizeof(GIfloat)), 
                                                GI_SSE_ALIGN_FLOAT);
    for(i = 0; i < iNumImages; ++i)
    {
        texture->image[i] = texture->data + (uintptr_t)texture->image[i];
