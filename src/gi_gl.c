...
static const GIchar *g_render_gs = 
    ...
    "#ifdef TEXTURE \n"
    "	vec4 vertex[4]; \n"
    "	vertex[0] = texture(geometryImage, gl_PositionIn[0].xy + sign[0] * offset[3].xy); \n"
    "	vertex[2] = texture(geometryImage, gl_PositionIn[0].xy + sign[2] * offset[3].xy); \n"
    "	vertex[1] = texture(geometryImage, gl_PositionIn[0].xy + sign[1] * offset[3].xy); \n"
    "	vertex[3] = texture(geometryImage, gl_PositionIn[0].xy + sign[3] * offset[3].xy); \n"
    "#endif \n"
...
void GIAPIENTRY giGLDrawGIM()
{
    ...
    if(bVTF)
    {
        pGL->_glUniform1i((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs, 
            "geometryImage") - 1, iGImage);
        if(bGS)
        {
            static const GIfloat fSign[8] = { -1.0f, 1.0f, -1.0f, -1.0f, 
                1.0f, -1.0f, 1.0f, 1.0f };
            pGL->_glUniform4fv((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs, 
                "offset") - 1, 4, fOffset);
            pGL->_glUniform2fv((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
                "sign")-1, 4, fSign);
        }
    }
    pGL->_glUniform4fv((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
    ...
    pGL->_glUniform4i((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
        "enabledAttribsSBTS")-1, iNormals, iColors, iScaleBiasNormal, iTwoSide);
    pGL->_glUniform4iv((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
        "enabledTextures")-1, 1, iTextures);
    pGL->_glUniform1i((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
        "normalImage")-1, iNImage);
    pGL->_glUniform1i((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
        "colorImage")-1, iCImage);
    pGL->_glUniform1iv((intptr_t)GIHash_find(&pGIMRenderer->uniform_locs,
        ...
        if(uiSemantic != GI_NONE)
        {
            ...
            case GI_GL_VERTEX:
                if(pMesh->asize[a] == 4)
                    ...
                    attribs[i].func = bFloat ? (GIvoid*)&glVertex3fv : (GIvoid*)&glVertex3dv;
                else
                    attribs[i].func = bFloat ? (GIvoid*)&glVertex2fv : (GIvoid*)&glVertex2dv;
                iSemanticAttrib[GI_GL_VERTEX_ATTRIB-GI_GL_SEMANTIC_BASE] |= 1;
                break;
            case GI_GL_NORMAL:
                attribs[i].func = bFloat ? (GIvoid*)&glNormal3fv : (GIvoid*)&glNormal3dv;
                break;
            case GI_GL_COLOR:
                if(pMesh->asize[a] == 4)
                          ...
                          GIuint num_g_out, GIuint num_defines, ...)
{
    ...
    GLuint uiFailed = 0;
    GLint iStatus;
    GLint i, iNum, iMaxLen, iLength, iSize, iNumStrings = (num_defines ? 3 : 2);
    GLenum uiType;
    GLchar* szName;
    const char* szSource[3] = {
#ifdef __APPLE__
        "#version 120\n"
#else
        "#version 140\n"
    ...
    if(iNum > 0)
    {
        ...
        for(i=0; i<iNum; ++i)
        {
            ...
            {
                const intptr_t attrLoc = gl->_glGetAttribLocation(shader->program, szName);
    ...
    if(iNum > 0)
    {
        ...
        for(i=0; i<iNum; ++i)
        {
            ...
            if(strncmp(szName, "gl_", 3))
            {
                ...
                const intptr_t uniformLoc = gl->_glGetUniformLocation(shader->program, szName);
...
void GIGLManager_init(GIGLManager *mgr)
{
    ...
    sscanf((const char*)glGetString(GL_VERSION), "%d.%d", &uiMajor, &uiMinor);
    mgr->gl_version = (uiMajor<<8) | uiMinor;
    const GIboolean v30 = mgr->gl_version >= 0x300;
    mgr->texture_3d = (mgr->gl_version >= 0x102);
    mgr->texture_cube_map = GI_TRUE;
    mgr->non_power_of_2 = GI_TRUE;
    ...
    if(mgr->glsl)
    {
        ...
        for(i=0; i<4; ++i)
        {
            ...
            else
            {
                ...
                if(mgr->gim_renderer[i]->program == 0)
                {
    ...
    if(mgr->color_buffer_float)
        mgr->_glClampColor = (PFNGLCLAMPCOLORPROC)(
            v30 ? GI_GL_PROC_ADDRESS(glClampColor) : GI_GL_PROC_ADDRESS(glClampColorARB));
    ...
    mgr->draw_buffers = GI_TRUE;
    if(mgr->draw_buffers)
    {
        /* get function pointers */
        mgr->_glDrawBuffers = (PFNGLDRAWBUFFERSPROC)
            GI_GL_PROC_ADDRESS(glDrawBuffers);
